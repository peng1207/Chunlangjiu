...
import UIKit
import SnapKit
typealias SPShopPlaceComplete = (_ model : SPPlaceModel?)->Void
class SPShopPlaceView : UIView{
    ...
    var selectBlock : SPShopPlaceComplete?
...
extension SPShopPlaceView : UITableViewDelegate,UITableViewDataSource{
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        let shopBrandCellID = "ShopPlaceCellID"
        ...
        if indexPath.row < sp_getArrayCount(array: self.dataArray) {
            ...
            cell?.textLabel?.text = sp_getString(string: model?.area_name)
            if model?.area_id == selectBrandID {
