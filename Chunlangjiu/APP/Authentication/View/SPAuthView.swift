...
class SPAuthView:  UIView{
    ...
    lazy var updateBtn : UIButton = {
        ...
        btn.setTitleColor(SPColorForHexString(hex: SP_HexColor.color_189cdd.rawValue), for: UIControlState.normal)
        btn.setTitleColor(SPColorForHexString(hex: SP_HexColor.color_b31f3f.rawValue), for: UIControlState.selected)
        btn.titleLabel?.font = sp_getFontSize(size: 15)
//        btn.isHidden = true
        btn.addTarget(self, action: #selector(sp_clickUpdate), for: UIControlEvents.touchUpInside)
        return btn
    }()
    var clickBlock : SPBtnClickBlock?
    var updateBlock : SPBtnClickBlock?
...
//            maker.width.equalTo(50)
            ...
            maker.centerX.equalTo(self.snp.centerX).offset(0)
            maker.top.equalTo(self.imgView.snp.bottom).offset(11)
        }
        self.updateBtn.snp.makeConstraints { (maker) in
            maker.width.greaterThanOrEqualTo(0)
            maker.height.equalTo(25)
            maker.centerX.equalTo(self.snp.centerX).offset(0)
            maker.top.equalTo(self.applyBtn.snp.bottom).offset(8)
        }
...
extension SPAuthView {
    ...
    @objc fileprivate func sp_clickUpdate(){
