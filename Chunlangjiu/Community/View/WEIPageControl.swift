...
//        }
        ...
        for index in 0 ..< numberOfPages {
            if index == localCurrentPage {//是当前点
                ...
                self.contentView.addSubview(currentPointView)
                ...
                self.contentView.addSubview(otherPointView)
    ...
    func exchangeCurrentView(oldSelectedIndex: NSInteger, newSelectedIndex: NSInteger) {//切换当前点和其他点
        let oldPointView = self.contentView.viewWithTag(1000 + oldSelectedIndex)
        let newPointView = self.contentView.viewWithTag(1000 + newSelectedIndex)
        ...
        if newSelectedIndex - oldSelectedIndex > 1 {//点击圆点，中间有跳过的点
            for index in oldSelectedIndex + 1 ..< newSelectedIndex {
                let view = self.contentView.viewWithTag(1000 + index)
